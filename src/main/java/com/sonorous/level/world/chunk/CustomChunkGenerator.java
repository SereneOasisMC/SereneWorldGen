    ...
    public void generateNoise(@NotNull WorldInfo worldInfo, @NotNull Random random, int chunkX, int chunkZ, ChunkData chunkData) {
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                ChunkUtils.HeightData data =  ChunkUtils.getHeightData(worldX, worldZ);

                Map<BiomeLayers, List<Material>> layers = NoiseMaster.getBiomeLayers(worldX, worldZ);

                for (int y = chunkData.getMinHeight(); y < ChunkUtils.Y_LIMIT && y < chunkData.getMaxHeight(); y++) {

                        if (y < data.primaryY) {
                            float distanceToFloor = Math.abs(y - data.primaryY);
                            if (distanceToFloor < ChunkUtils.PRIMARY_HEIGHT) {
                                chunkData.setBlock(x, y, z, layers.get(BiomeLayers.PRIMARY).get(random.nextInt(layers.get(BiomeLayers.PRIMARY).size())));
                            }
                        }

//                        else if (y < data.secondaryY) {
//                            float distanceToFloor = Math.abs(y - data.secondaryY);
//                            if (distanceToFloor < ChunkUtils.SECONDARY_HEIGHT) {
...
//                        }
    ...
    public void generateBedrock(@NotNull WorldInfo worldInfo, @NotNull Random random, int chunkX, int chunkZ, @NotNull ChunkGenerator.ChunkData chunkData) {
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                Map<BiomeLayers, List<Material>> layers = NoiseMaster.getBiomeLayers(chunkX * 16 + x, chunkZ * 16 + z);
                for (int y = chunkData.getMinHeight(); y < ChunkUtils.Y_LIMIT && y < chunkData.getMaxHeight(); y++) {
...
//                    if (y < currentY) {
//                        float distanceToSurface = Math.abs(y - currentY);
//                        if (distanceToSurface > ChunkUtils.PRIMARY_HEIGHT) {
...
//                                List<Material> neighbourBlocks = Arrays.asList(chunkData.getType(Math.max(x - 1, 0), y, z), chunkData.getType(x, Math.max(y - 1, 0), z), chunkData.getType(x, y, Math.max(z - 1, 0)));
//
//                                if (random.nextFloat() < 0.0002) {
//                                    neighbour = layers.get(BiomeLayers.ORES).get(Math.min(random.nextInt(layers.get(BiomeLayers.ORES).size()), random.nextInt(layers.get(BiomeLayers.SECONDARY).size())));
//                                }
//
//                                if ((!Collections.disjoint(neighbourBlocks, layers.get(BiomeLayers.ORES)))) {
//                                    for (Material neighbourBlock : neighbourBlocks) {
//                                        if (layers.get(BiomeLayers.ORES).contains(neighbourBlock) && random.nextFloat() < -0.01 * layers.get(BiomeLayers.SECONDARY).indexOf(neighbourBlock) + 0.4) {
